...
pathname_data=[pathname_MS_data 'whole_brain_imaging_long_term_adaptation\raw_behavior\'];
% final path to pre-processed data
pathname_behavior=[pathname_MS_data 'whole_brain_imaging_long_term_adaptation\behavior\'];
...
for f=16%1:num_fish
    ...
    meta.z_step = meta.piezo_z_amp/meta.num_planes;
    meta.rez = 0.6;
    ...
    if meta.lag_condition==0
        meta.group = 'control';
    else
        if nanmean(trials.bout_duration.first(61:70)) - nanmean(trials.bout_duration.first(21:30))<-0.04
           meta.group = 'lag-trained adapting';
        else
            meta.group = 'lag-trained non-adapting';
        end
    end
    disp(meta.group);
